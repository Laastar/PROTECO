...
public class Traductor
{
	...
	public static void main(String[] args) 
	{
		...
		try
		{
			ObjectOutputStream españolIngles = new ObjectOutputStream(new FileOutputStream(new File(System.getProperty("user.dir") + "/EspañolIngles")));
			ObjectOutputStream inglesEspañol = new ObjectOutputStream(new FileOutputStream(new File(System.getProperty("user.dir") + "/InglesEspañol")));
			españolIngles.writeObject(palabrasEsp);
			inglesEspañol.writeObject(palabrasIng);
			españolIngles.close();
			inglesEspañol.close();
		}
		catch(Exception e)
		{
			System.out.println("Error al crear traductor");
		}
		...
		while(eleccion != 4)
		{
			...
			{
				Traduccion.InglesEspañol();
			}
			else if (eleccion == 2)
			{
				Traduccion.EspañolIngles();
			}
			else if (eleccion == 3)
			{
				DarAltaPalabra.seleccionTipo();
			}
			else
			{
